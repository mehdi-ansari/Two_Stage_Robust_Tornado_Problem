...
class stabbing_line:
    def __init__(self, C, bldngs, coefficient, radius, max_dist):
        self.C = C
        self.bldngs = bldngs
    ...
    def events(self):
        ...
        for B in bldngs:
            ...
            if Euclidean_distance(B, C) <= Q:    #related to Length of tornado!! it means the length of tornado is Q-2*Delta
                if Euclidean_distance(B, C) > epsilon:
    ...
    def max_overlap(self):
        ...
        if len(A) == 0:
            A = [(0, 0, 0)]
            track = [0]
        return A[np.argmax(track)], max(track)
